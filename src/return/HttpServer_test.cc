...
void onRequest(const HttpRequest& req, HttpResponse* resp)
{
  ...
  {  
    ...
    resp->setContentType("text/plain");
    resp->addHeader("Server", "Muduo");  
    resp->setBody(dao.AdminLogin(req.body()));
    resp->setCloseConnection(true);
  }
     else if (req.path() == "/SuperAdminLogin")
  {  
    Dao dao;
    resp->setStatusCode(HttpResponse::k200Ok);
    resp->setStatusMessage("OK");
    resp->setContentType("text/plain");
    resp->addHeader("Server", "Muduo");  
    resp->setBody(dao.SuperAdminLogin(req.body()));
    resp->setCloseConnection(true);
  }
  ...
  {  
    ...
    resp->setContentType("text/plain");
    resp->addHeader("Server", "Muduo");  
    resp->setBody(dao.TodaySubmit(req.body()));
    resp->setCloseConnection(true);
  }
  ...
  { 
    ...
    resp->setContentType("text/plain");
    resp->addHeader("Server", "Muduo");  
    resp->setBody(dao.ChangePwd(req.body()));
    resp->setCloseConnection(true);
  }
  ...
  { 
    ...
    resp->setContentType("text/plain");
    resp->addHeader("Server", "Muduo");  
    resp->setBody(dao.GetSubmitLog(req.body()));
    resp->setCloseConnection(true);
  }
